...
namespace dxvk {
  ...
  D3D9CommonTexture::D3D9CommonTexture(
    ...
    m_mapMode        = DetermineMapMode(m_desc.Pool == D3DPOOL_SYSTEMMEM && pSharedHandle);
  ...
  bool D3D9CommonTexture::CreateBufferSubresource(UINT Subresource, bool Initialize) {
    ...
    if (Initialize) {
      if (m_lockingData[Subresource]) {
        m_lockingData[Subresource].Map();
        memcpy(m_mappedSlices[Subresource].mapPtr, m_lockingData[Subresource].Ptr(), GetMipSize(Subresource));
      } else {
  ...
  D3D9_COMMON_TEXTURE_MAP_MODE D3D9CommonTexture::DetermineMapMode(bool hasInitialData) const {
    if (m_desc.Format == D3D9Format::NULL_FORMAT)
    return D3D9_COMMON_TEXTURE_MAP_MODE_NONE;

#ifdef D3D9_ALLOW_UNMAPPING
    if (m_device->GetOptions()->resourceMemory != 0 && m_desc.Pool != D3DPOOL_DEFAULT && !hasInitialData)
